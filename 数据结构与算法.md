...
特点：查询非常快，但是删除和插入较慢，需要元素移动保证表的连续
...
队列是一种特殊的线性表， 特殊之处在于它**只允许在表的前端（front） 进行删除操作， 而在表的后端（rear） 进行插入操作**， 和栈一样， 队列是一种**操作受限制的线性表。 进行插入操作的端称为队尾， 进行删除操作的端称为队头**
...
![image-20241102120816980](./数据结构与算法Img/image-20241102120816980.png)
...
![image-20241102121039893](./数据结构与算法Img/image-20241102121039893.png)

**树存储方式分析**：**能提高数据存储， 读取的效率**， 比如可以使用二叉树既可以保证数据检索速度， 同时也可以保证数据的插入， 删除， 修改的速度。是插入删除查询速度都中和的一种存储方式

我们可以基于树的结构提高数组的缺点进行优化，也可以对链表的缺点进行优化

![image-20241102121802783](./数据结构与算法Img/image-20241102121802783.png)
...
二叉树**特点**：每个结点最多只能有两棵子树， 且有左右之分  

### 3. 树示意图  

![image-20241102123619498](./数据结构与算法Img/image-20241102123619498.png)
...
树有很多种， 每个结点最多只能有两个子结点的一种形式称之为二叉树二叉树分为左结点和右结点
![image-20241102135954753](./数据结构与算法Img/image-20241102135954753.png)

1. 如果该二叉树的所有叶子结点都在最后一层， 并且结点总数是 $2^n-1$， $n$ 是层数， 则我们称之为**满二叉树**
   ![image-20241102140009633](./数据结构与算法Img/image-20241102140009633.png)
2. 如果该二叉树的所有叶子结点都在最后一层或者倒数第二层， 而且最后一层的叶子结点在左边连续， 倒数第二层的叶子结点在右边连续， 我们称之为**完全二叉树**
   ![image-20241102140021427](./数据结构与算法Img/image-20241102140021427.png)
...
| 成员变量 | 1.`public Node left`：记录左子结点<br />2.`public Node righ`t：记录右子结点<br />3.`public Key key`：存储键<br />4.`public Value value`：存储值 |

**二叉树 API 设计  :**

| 类名     | `Binary<Tree extends Comparable<key>, Value value>`          |
| -------- | ------------------------------------------------------------ |
| 构造方法 | `BinaryTree()`：创建BinaryTree对象                           |
| 成员变量 | 1.`private Node root`:记录根结点<br />2.`privateint N`：记录树中元素的个数 |
| 成员方法 | 1.`public void put(Key key，Value value)`:向树中插入—个键值对<br />2.`private Node put)Node x，Key key,Value val)`：给指定树x上，添加键一个键值对，并返回添加后的新树<br />3.`public Value get(Key key)`：根据key，从树中找出对应的值<br />4.`private Value get(Node x, Keykey)`：从指定的树x中，找出key对应的值<br />5.`public void delete(Key key)`:根据key，删除树中对应的键值对<br />6.`private Node delete(Node x，Key key)`：删除指定树x上的键为key的键值对，并返回删除后的新树<br />7.`public int size()`：获取树中元素的个数 |
...
[![image-20241102172258521](./数据结构与算法Img/image-20241102172258521.png)](./数据结构与算法Img/image-20241102172258521.png)
...
3. 删除右子树中的最小结点
4. 让被删除结点的左子树称为最小结点 minNode 的左子树，让被删除结点的右子树成为最小结点 minNode 的右子树 
5. 让被删除结点的父节点指向最小结点 minNode

<img src="./数据结构与算法Img/无标题.png" alt="无标题" style="zoom:80%;" />
...
public class TreeTest {
    public static void main(String[] args) {
        ...
        // 前中后序遍历查找
        System.out.println("binaryTree.preSearch(2) = " + binaryTree.preSearch(2)); // Node{no=2, name=小李, left=...
        System.out.println("binaryTree.infixSearch(3) = " + binaryTree.infixSearch(3)); // binaryTree.infixSearch(3) = Node{no=3, name=小周, left=...
        System.out.println("binaryTree.postSearch(2) = " + binaryTree.postSearch(2)); // binaryTree.postSearch(2) = Node{no=2, name=小李, left=...
    }
}
```
...
1. 它是**完全二叉树**，除了树的最后一层结点不需要是满的，其它的每一层从左到右都是满的，如果最后一层结点不是满的，那么要求左满右不满。
   ![image-20241203204814679](./数据结构与算法Img/image-20241203204814679.png)

2. 它**通常用数组来实现**。
   具体方法就是将二叉树的结点按照层级顺序放入数组中，根结点在位置 1(根节点不能为0，如果为0则下面的公式不生效了)，它的子结点在位置 2 和 3，而子结点的子结点则分别在位置 4,5,6 和 7，以此类推
   ![image-20241203204829201](./数据结构与算法Img/image-20241203204829201.png)

   如果一个结点的位置为`k`，则它的父结点的位置为`[k/2]`,而它的两个子结点的位置则分别为 `2k` 和 `2k+1`。这样，**在不使用指针的情况下，我们也可以通过计算数组的索引在树中上下移动**：从 `a[k]`向上一层，就令` k` 等于 `k/2`,向下一层就令 `k` 等于 `2k` 或 `2k+1`。

3. 每个结点都大于等于它的两个子结点。这里要注意**堆中仅仅规定了每个结点大于等于它的两个子结点，但这两个子结点的顺序并没有做规定**，跟我们之前学习的二叉查找树是有区别的。
   ![image-20241203204936359](./数据结构与算法Img/image-20241203204936359.png)

   如上图这就是堆结构， **所有根节点的值永远比左右子树的大， 那么就可以看出， 整棵树的根节点， 他的值是整个堆中最大的**。
   同时我们也发现没有直接父子关系的节点他们的值没有完全地关系， 就像第二层的 33 和第三层的 45 以及 20， 没有规定第三层的元素值必须小于第二层， 只要满足根节点比自己左右子树节点的值大即可。

#### 6.2 堆 API 设计

| 类名     | `Heap<T extendsComparable<T>>`                               |
| -------- | ------------------------------------------------------------ |
| 构造方法 | `Heap(int capacity)`：创建容量为capacity的Heap对象           |
| 成员方法 | 1.`private boolean less(int i, int j)`：判断堆中索引`i`处的元素是否小于索引`j`处的元素<br />2.`private void exct(int i, int j)`：交换堆中`i`索引和j索引`j`处的值<br />3.`public T delMax()`:删除堆中最大的元素，并返回这个最大元素<br />4.`public void insert(T t)`：往堆中插入—个元素<br />5.`private void swim(int k)`：使用上浮算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />6.`private void sink(int k)`：使用下沉算法，使索引k处的元素能在堆中处于一个正确的位置 |
...
![image-20241205190907351](./数据结构与算法Img/image-20241205190907351.png)

在堆中插入元素后， 还要满足堆的特性， 保持数据层次上的顺序(由小到大， 或者由大到小)。对于一个大顶堆， 如果新插入的元素在堆的最底层， 需要对这个元素和上层的元素比较大小，如果比上层的元素大则需要**上浮**操作， 也就是一层一层的进行比较交换， 如下图所示:  

![image-20241205192912200](./数据结构与算法Img/image-20241205192912200.png)

从堆中删除元素和插入元素同理， 需要判断要删除的元素和上下层的大小关系， 其实可以把上面插入的操作反过来理解， 对应的操作是**下沉**操作， 如下图所示:  

![image-20241205193021463](./数据结构与算法Img/image-20241205193021463.png)

#### 6.4 堆的实现代码

这里所指的堆又叫[最大优先队列](####7.1 最大优先队列)，可以删除并返回最大值
...
System.out.println("使用堆进行排序后的结果arrays = " + Arrays.toString(arrays)); // 使用堆进行排序后的结果arrays = [A, E, G, H, I, N, O, P, R, S, T]
String[] source = {"S", "G", "I", "E", "N", "H", "O", "A", "T", "P", "R"};
String[] heap1 = new String[source.length + 1];
HeapSort.createHeap(heap1, arrays);
System.out.println("根据源数组生成的堆数组heap1 = " + Arrays.toString(heap1)); // 根据源数组生成的堆数组heap1 = [null, T, S, O, R, I, N, G, P, H, E, A]
```

### 7. 优先队列

普通的队列是一种**先进先出的数据结构**，元素在**队列尾追加**，而从**队列头删除**。

在某些情况下，我们可能需要**找出队列中的最大值或者最小值**，例如使用一个队列保存计算机的任务，一般情况下计算机的任务都是有优先级的，我们需要在这些计算机的任务中找出优先级最高的任务先执行，执行完毕后就需要把这个任务从队列中移除。普通的队列要完成这样的功能，需要每次遍历队列中的所有元素，比较并找出最大值，效率不是很高，这个时候，我们就可以使用一种特殊的队列来完成这种需求，优先队列。  

![image-20241207140155020](./数据结构与算法Img/image-20241207140155020.png)

优先队列按照其**作用不同**，可以**分为**以下两种：
...
堆这种结构是可以**方便的删除最大的值**，所以，接下来我们可以基于堆区实现最大优先队列
...
| 成员方法 | 1.`private boolean less(int i, int j)`：判断堆中索引`i`处的元素是否小于索`j`处的元素<br />2.`private void exch(int i, int j)`：交换堆中`i`索引和`j`索引处的值<br />3.`public T delMax()`：删除队列中最大的元素，并返回这个最大元素<br />4.`public void insert(T t)`：往队列中插入一个元素<br />5.`private void swim(int k)`：使用上浮算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />6.`private void sink(int k)`：使用下沉算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />7.`public int size()`)：获取队列中元素的个数<br />8.`public boolean isEmpty()`):判断队列是否为空 |
| 成员变量 | 1.`private T[] imtes`：用来存储元素的数组<br />2.`private int N`：记录堆中元素的个数 |
我们前面学习堆的时候，堆中存放数据元素的数组要满足都满足如下特性：

1. **最大的**元素放在数组的**索引 `1`** 处。
2. 每个结点的数据总是**大于等于它的两个子结点**的数据。

![image-20241207141252105](./数据结构与算法Img/image-20241207141252105.png)

**其实我们之前实现的堆可以把它叫做最大堆**

前面的代码：[代码实现](####6.4 堆的实现代码)

#### 7.2 最小优先队列  

最小优先队列实现起来也比较简单，我们同样也可以基于堆来完成最小优先队列。

，我们可以**用相反的思想实现最小堆**，让堆中存放数据元素的数组满足如下特性：

1. **最小的**元素放在数组的**索引` 1`** 处。
2. 每个结点的数据总是**小于等于它的两个子结点**的数据。

![image-20241207141336622](./数据结构与算法Img/image-20241207141336622.png)
...
| 成员方法 | 1.`private boolean less(int i, int j)`：判断堆中索引`i`处的元素是否小于索`j`处的元素<br />2.`private void exch(int i, int j)`：交换堆中i索引和j索引处的值<br />3.`public T delMin()`：删除队列中最小的元素，并返回这个最小元素<br />4.`public void insert(T t)`：往队列中插入一个元素<br />5.`private void swim(int k)`：使用上浮算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />6.`private void sink(int k)`：使用下沉算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />7.`public int size()`)：获取队列中元素的个数<br />8.`public boolean isEmpty()`):判断队列是否为空 |
| 成员变量 | 1.`private T[] imtes`：用来存储元素的数组<br />2.`private int N`：记录堆中元素的个数 |
...
while (!minQueue.isEmpty()) {
    System.out.println("删除并返回的最小值：" + minQueue.delMin()); // A C E P R S T O U 
}
```

#### 7.3 索引优先队列

**之前实现的最大优先队列和最小优先队列**，他们可以分别快速访问到队列中最大元素和最小元素，但是他们有一个缺点，就是**没有办法通过索引访问已存在于优先队列中的对象，并更新它们**。
...
存储数据时，给每一个数据元素关联一个整数，例如 `insert(int k, T t)`,我们可以看做 k 是 t 关联的整数，那么我们的实现需要通过 k 这个值，快速获取到队列中 t 这个元素，此时有个 k 这个值需要具有唯一性。

最直观的想法就是我们可以用一个 `T[] items` 数组来保存数据元素，在 `insert(int k, T t)`完成插入时，可以把 k 看做是items 数组的索引，把 t 元素放到 items 数组的索引 k 处，这样我们再根据 k 获取元素 t 时就很方便了，直接就可以拿到`items[k]`即可

![image-20241207145402890](./数据结构与算法Img/image-20241207145402890.png)

*步骤二：*  

步骤一完成后的结果，虽然我们给每个元素关联了一个整数，并且可以使用这个整数快速的获取到该元素，但是， items 数组中的元素顺序是随机的，并不是堆有序的，所以，为了完成这个需求，我们可以**增加一个数组 `int[] pq`,来保存每个元素在 items 数组中的索引**，pq 数组需要堆有序，也就是说，pq[1]对应的数据元素 items[pq[1]]要小于等于 pq[2]和 pq[3]对应的数据元素 items[pq[2]]和 items[pq[3]]。  

![image-20241207145441991](./数据结构与算法Img/image-20241207145441991.png)

*步骤三：*

通过步骤二的分析，我们可以发现，其实**我们通过上浮和下沉做堆调整的时候，其实调整的是 pq 数组**。如果需要对 items 中的元素进行修改，比如让 items[0]=“H”,那么很显然，我们需要对 pq 中的数据做堆调整，而且是调整pq[9]中元素的位置。

但现在就会遇到一个问题，我们修改的是 items 数组中 0 索引处的值，**如何才能快速的知道需要挑中 pq[9]中元素的位置呢？答案：借助qp数组**

最直观的想法就是遍历 pq 数组，拿出每一个元素和 0 做比较，如果当前元素是 0，那么调整该索引处的元素即可，但是效率很低。

![image-20241207145513125](./数据结构与算法Img/image-20241207145513125.png)

**三个数组的关系：**

[![索引优先队列](./数据结构与算法Img/索引优先队列.png)](./数据结构与算法Img/索引优先队列.png)

设计：

| 类名 | `IndexMinPriorityQueue<T exteneds Comparable>` |
| --- | --- |
| 构造方法 | `IndexMinPriorityQueue(int capacity)`：创建容量为capacity的IndexMinPriorityQueue对象 |
| 成员方法 | 1.`privatebooleanless(int i, int j)`：判断堆中索引`i`处的元素是否小于索引`j`处的元素<br />2.`private void exch(int i, int j)`：交换堆中`i`索引和`j`索引处的值<br />3.`public int delMin()`：删除队列中最小的元素，并返回该元素关联的索引<br />4.`public void insert(int i, T t)`：往队列中插入一个元素，并关联索引i<br/> 5.`private void swim(int k)`：使用上浮算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />6.`private void sink(int k)`：使用下沉算法，使索引`k`处的元素能在堆中处于一个正确的位置<br />7.`public int size()`：获取队列中元素的个数<br />8.`public boolean isEmpty()`:判断队列是否为空<br />9.`publicbooleancontains(intk)`:判断`k`对应的元素是否存在<br />10.`public void changeltem(int i, T t)`：把与索引`i`关联的元素修改为为`t`<br />11.`public int minIndex()`:最小元素关联的索引<br />12.`public void delete(int i)`：删除索引`i`关联的元素 |
| 成员变量 | 1.`private T[] imtes`：用来存储元素的数组<br />2.`private int[] pg`：保存每个元素在items数组中的索引l，pq数组需要堆有序 <br />3.`private int[] gp`：保存gp的逆序，pg的值作为索引，pg的索引作为值<br />4.`private int N`：记录堆中元素的个数 |
...
while (!minQueue.isEmpty()) {
	System.out.println("删除并返回的最小值：" + string[minQueue.delMin()]); // A C E P R S T U
}
```
...
1. 二叉树需要加载到内存的， 如果二叉树的节点少， 没有什么问题， 但是如果二叉树的节点很多(比如 10 亿)， 就存在问题了。
2. 在构建二叉树时， 需要多次进行 i/o 操作(海量数据存在数据库或文件中)， 节点海量， 构建二叉树时， 速度有影响。
3. 节点海量， 也会造成二叉树的高度很大， 会降低操作速度。

### 9. 多叉树

如B树、2-3树(2-3-4树等)、B+树、B*树等

### 10. B 树介绍

B 树通过重新组织节点， **降低树的高度， 并且减少 i/o 读写次数来提升效率**

![image-20241210145409070](./数据结构与算法Img/image-20241210145409070.png)
...
![image-20241210145922718](./数据结构与算法Img/image-20241210145922718.png)
...
2. 有两个子节点的节点叫二节点， 二节点要么没有子节点， 要么有两个子节点. 
3. 有三个子节点的节点叫三节点， 三节点要么没有子节点， 要么有三个子节点
4. 对于三节点的子树的值大小仍然遵守(BST 二叉排序树【左小右大】)(中间的节点处于父节点两个值的中间数 )的规则

除了 2-3 树， 还有 2-3-4 树等， 概念和 2-3 树类似， 也是一种 B 树

![image-20241210150005402](./数据结构与算法Img/image-20241210150005402.png)
...
2-3 树和 2-3-4 树， 他们就是 B 树(英语： B-tree 也写成 B-树)， 这里我们再做一个说明， 我们在学习 Mysql 时， 经常听到说某种类型的索引是基于 B 树或者 B+树的， 如图:

![image-20241210150134287](./数据结构与算法Img/image-20241210150134287.png)
...
![image-20241210150341811](./数据结构与算法Img/image-20241210150341811.png)

1. B+树的搜索与 B 树也基本相同， **区别是 B+树只有达到叶子结点才命中**（B 树可以在非叶子结点命中） ， 其性能也等价于在关键字全集做一次二分查找
2. 所有关键字都出现在叶子结点的链表中（即**数据只能在叶子节点**【也叫稠密索引】 ） ， 且链表中的关键字(数据)恰好是有序的。
3. 不可能在非叶子结点命中
...
![image-20241210150532719](./数据结构与算法Img/image-20241210150532719.png)

1. B\*树定义了非叶子结点关键字个数至少为$\frac{2M}{3}$【M为度】， 即块的最低使用率为 2/3， 而 B+树的块的最低使用率为的 1/2。
...
![image-20241210151038518](./数据结构与算法Img/image-20241210151038518.png)
...
- 树的带权路径长度
  ...
  **权值越大的结点距离根结点越近的二叉树才是最优二叉树(赫夫曼树)**
  **wpl 最小的就是赫夫曼树**

#### 13.4 转化赫夫曼示意图

![image-20241210155107539](./数据结构与算法Img/image-20241210155107539.png)

需求：给定一个数列`{13,7,8,29,6,1}`要求转成一颗赫夫曼树

思路分析：

1. 从小到大进行排序`{1,6,7,8,13,29}`， 每个数据都是一个节点， 每个结点可以看成是一颗简单的二叉树

2. 从排好序的树列中取出根结点最小的两颗二叉树`{1,6}`

3. 该新的二叉树根结点权值是前面两颗二叉树节点权值之和。

   `1,6,7,8,13,29`
   ![image-20241210155934446](./数据结构与算法Img/image-20241210155934446.png)

4. 将这颗新的二叉树的根结点的权值大小与数列中剩余的元素再次排序，
   `{7,7,8,13,29}`
   ![image-20241210155901519](./数据结构与算法Img/image-20241210155901519.png)

   不断重复以上步骤，
   `{8,13,14,29}`
   ![image-20241210160206239](./数据结构与算法Img/image-20241210160206239.png)
   `{14,21,29}`

   ![image-20241210160408033](./数据结构与算法Img/image-20241210160408033.png)
   `{29,35}`

   ![](./数据结构与算法Img/image-20241210161027865.png)

   直到数列中所有的数据被处理， 即可得到一颗赫夫曼树

案例 `{13,7,8,29,6,1}` 组成的赫夫曼树：
![](./数据结构与算法Img/image-20241210161027865.png)

`{1,3,6,13,7,8,29}`组成的赫夫曼树 ：

![image-20241210151531317](./数据结构与算法Img/image-20241210151531317.png)

#### 13.5 赫夫曼代码实现

需求：给定一个数列`{13,7,8,29,6,1}`要求转成一颗赫夫曼树

实现的原理图[见上](####13.4 转化赫夫曼示意图)
